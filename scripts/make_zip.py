...
def  main():
  ...
  installer = "\installer\Tablitsa Installer.exe"
   
  if demo:
    installer = "\installer\Tablitsa Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Tablitsa manual.pdf" 
  ]
  ...
  ZIPNAME = "Tablitsa-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Tablitsa-v" + FULLVERSIONSTR + "-win-demo.zip"
